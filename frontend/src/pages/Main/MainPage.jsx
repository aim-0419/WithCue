...
import { fetchWeeklyAccuracyFromServer } from "../../utils/accuracyApi";

export default function MainPage() {
  const navigate = useNavigate();
  const { pathname, state } = useLocation();
  const [weeklyAccuracyData, setWeeklyAccuracyData] = useState(() => toWeeklyChartData());
  ...
  useEffect(() => {
    ...
    async function loadWeeklyAccuracy() {
      try {
        ...
        if (!mounted) return;
        setWeeklyAccuracyData(Array.isArray(payload.items) ? payload.items : toWeeklyChartData());
  ...
  const finalAccuracy = state?.finalAccuracy ?? serverLatestAccuracy ?? readLatestAccuracy() ?? 0;
