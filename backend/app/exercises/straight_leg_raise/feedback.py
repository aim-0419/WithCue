# Straight Leg Raise(다리 들기) 운동의 자세 비교 결과를 분석해 사용자에게 전달할 피드백 목록을 생성하는 모듈.
# DTW 비교 결과(유사도, 관절별 오차 등)를 읽어 "몸통 흔들림", "다리 높이 부족" 등
...
# Straight Leg Raise(오른쪽 다리) DTW 비교 결과를 분석해 자세 문제 항목 리스트를 반환한다.
# compare_result: DTW 엔진이 반환한 딕셔너리 (유사도, 관절별 오차, 동작 단계 등 포함)
# 반환: 감지된 문제 항목(issue) 딕셔너리의 리스트. 문제가 없으면 빈 리스트.
def extract_straight_leg_raise_right_issues(compare_result: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 다리를 들어올리는 구간에서 발목 높이 또는 엉덩이 굽힘 범위가 부족한지 확인
    if (
    ...
    # 다리를 내리는 구간에서 힘을 빼고 빠르게 내리는지 확인
