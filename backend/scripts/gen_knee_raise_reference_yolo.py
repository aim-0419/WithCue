# 무릎 들기 정상 영상에서 DTW 레퍼런스 JSON 생성 (YOLO)
import os
import json
import cv2
import numpy as np
...
from app.exercises.knee_raise.features import (
...
def main():
    ...
    while True:
        ...
        results = model(frame_bgr, verbose=False, device="cpu", imgsz=640)

        if frame_idx % 24 == 0 and results:
            annotated = results[0].plot()
            cv2.imwrite(os.path.join(debug_dir, f"frame_{frame_idx:05d}.jpg"), annotated)
    ...
    for i in range(2, len(arr) - 2):
        if arr[i] > 50 and arr[i] >= arr[i - 1] and arr[i] >= arr[i + 1]:
            peak_idx = i
            break
    if peak_idx is None:
    ...
    start_idx = 0 if peak_idx <= 2 else int(np.argmin(arr[:peak_idx]))
    end_idx = n - 1 if peak_idx >= n - 3 else peak_idx + int(np.argmin(arr[peak_idx:]))

    if end_idx <= start_idx:
        raise RuntimeError(f"rep 구간이 비정상입니다. start={start_idx}, peak={peak_idx}, end={end_idx}")

    start_idx = max(0, start_idx - 3)
    end_idx = min(len(sequence) - 1, end_idx + 3)
